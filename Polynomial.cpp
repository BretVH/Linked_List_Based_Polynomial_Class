...
Polynomial Polynomial::operator+(const Polynomial& p) const
{
	...
	Term* newCurr = sum.head;  //create pointer to new Polynomial's head
	for( ; (startL <= sizeL) && (startR <= sizeR); newCurr=newCurr->next)  //iterate over this and p and new polynomial
	{
...
Polynomial& Polynomial::operator+=(const Polynomial& p)
{
	...
	int startL = 1;
	int startR = 1;
	Term* newCurr = head->next;
	for( ; (startL <= sizeL) && (startR <= sizeR); newCurr=newCurr->next)
	{
...
bool Polynomial::insert(Term* prev, const double newCoefficient, const int power)
{
	if(prev == NULL)//Insert fails
		return false;
	Term* found = new Term;  //create new Term to insert
	found->coeff = newCoefficient;
	if(newCoefficient == 0)
	{
  ...
  // insert found before prev
	if(prev == head && size == 0)
	{
	  // insert found between head and 1st term
		found->next = head->next;
		head->next->prev = found;
		head->next = found;
		found->prev = head;
	}
	else if(size > 0 && prev == head)
	{
		//insert found after last term
